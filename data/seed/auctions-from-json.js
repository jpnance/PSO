/**
 * Seed auction and contract transactions from auctions.json
 * 
 * Creates:
 *   - auction-ufa transactions (player won at auction)
 *   - contract transactions (contract terms signed)
 * 
 * Timestamps:
 *   - Auction wins: auction day at noon ET
 *   - Contract signings: contract due date at noon ET
 * 
 * Usage:
...
async function seed() {
	...
	if (args.clear && !args.dryRun) {
		...
		var contractResult = await Transaction.deleteMany({ type: 'contract' });
		console.log('  Deleted ' + contractResult.deletedCount + ' contract transactions');
		console.log('');
	}
	...
	// Load drafts.json to identify players who were drafted (not auctioned)
	var drafts = JSON.parse(fs.readFileSync(DRAFTS_FILE, 'utf8'));
	var draftedPlayers = new Set();
	drafts.forEach(function(d) {
		if (d.passed) return; // Passed picks don't count
		if (d.sleeperId) {
			draftedPlayers.add(d.sleeperId + '|' + d.season);
		} else if (d.playerName) {
			draftedPlayers.add(d.playerName.toLowerCase() + '|' + d.season);
		}
	});
	console.log('Loaded ' + drafts.length + ' draft entries (' + draftedPlayers.size + ' drafted players)');
	console.log('');
	...
	for (var si = 0; si < seasonYears.length; si++) {
		...
		for (var ai = 0; ai < seasonAuctions.length; ai++) {
			...
			// Skip players who were drafted this season (they appear in auctions.json
			// because they got a contract, but their acquisition was via draft, not auction)
			var isDrafted = false;
			if (entry.sleeperId) {
				isDrafted = draftedPlayers.has(entry.sleeperId + '|' + season);
			} else if (entry.name) {
				isDrafted = draftedPlayers.has(entry.name.toLowerCase() + '|' + season);
			}
			if (isDrafted) {
				stats.draftedSkipped++;
				continue;
			}
			...
			try {
				...
				// Create contract transaction if we have contract terms
				if (entry.startYear && entry.endYear) {
					var contractTs = new Date(contractTimestamp.getTime() + (ai * 1000));
		...
		console.log(season + ': ' + seasonAuctionCount + ' auctions, ' + seasonContractCount + ' contracts');
	}
	...
	console.log('  Drafted (skipped): ' + stats.draftedSkipped);
