...
function getPaneDefaults(viewId) {
    if (viewId.startsWith('pos-')) return { contractView: 'available', sortBy: 'ppg', showCount: false };
    ...
    if (viewId === 'rookies') return { contractView: 'available', sortBy: 'ppg', showCount: false };
    if (viewId === 'watchlist') return { contractView: 'all', sortBy: 'ppg', showCount: false };
    if (viewId.startsWith('bye-')) return { contractView: 'available', sortBy: 'ppg', showCount: false };
...
function renderPanes() {
    ...
    win.panes.forEach(function(paneData, paneIndex) {
        ...
        var filterHtml = '<div class="pane__filter" data-pane-index="' + paneIndex + '">' +
            ...
            '<button class="pane__filter-btn' + (paneData.contractView === 'available' ? ' pane__filter-btn--active' : '') + '" data-filter="available">Available</button>' +
        ...
        var isAvailableOnly = paneData.contractView === 'available';
        ...
        if (isAvailableOnly) tableClasses.push('player-table--no-salary', 'player-table--no-owner');
